...
| revoke         | bool/string | -                                                           | 真值（JSON `true`/非零数字，query 的 `1`/`true`/`yes`/`on` 或裸 `?revoke`）进入**撤回模式**：用 `id`（原通知 notifyId，须正整数）撤回该 key 下鸿蒙设备上尚未点击/未下发的通知，调用华为 v1 `messages:revoke`；忽略其它所有推送参数、不写监控流；仅鸿蒙生效（iOS 无远程撤回 API），无鸿蒙记录返回 400；需在 `harmony/harmony_certs.go` 配置应用级 `clientID`，详见 [通知撤回](#通知撤回-仅鸿蒙) |
...
- 模式支持 `CBC`、`ECB`、`GCM`；
- CBC/ECB 固定使用 `PKCS7`，GCM 固定使用 `NoPadding`；
- CBC 的 IV 必须为 16 个 UTF-8 字节，GCM 的 IV 必须为 12 个 UTF-8 字节，并由发送端为每条消息重新随机生成；ECB 不使用 IV，可省略 `iv`；
- GCM 不使用 AAD，使用 16 字节认证标签；发送端需要将认证标签拼接在密文末尾，再把完整的 `ciphertext || authTag` 编码为 Base64；
- 解密结果必须是 JSON 对象。解密失败时客户端显示安全占位内容，不会尝试其它服务器的 Key；
- Key 按服务器独立保存在 HarmonyOS Asset Store 中，禁止设备克隆和云同步，不上传及时通知服务器，也不写入普通 Preferences、消息缓存或日志。
- Harmony 客户端的 Key 输入框使用明文显示，也可点击重置图标调用系统密码学安全随机数生成器，按当前 AES 算法生成 16、24 或 32 字节的可打印 Key。
- 随机生成的新 Key 在点击保存前只是候选值。已有配置的 Key、算法或模式发生变化时，客户端会在保存前提醒用户备份旧 Key 并同步更新发送端；确认保存后，使用旧配置加密的历史密文不保证继续可解密。

`ciphertext` 为空时保持现有普通通知行为。没有扩展通知权益时，系统通知栏无法在展示前显示解密后的内容。

Harmony 客户端将远程消息历史作为待迁移队列：首次进入及之后每次轮询都从 `after=0` 检查远程记录。每页通知先完成解密，再应用 `isArchive`/`ttl`：缺省或 `isArchive=1` 且尚未过期的通知会将明文、原始 `ciphertext`/`iv`、加密标记与绝对过期时间写入本地数据库；显式关闭归档或同步时已经过期的通知不落本地。TTL 剩余不足 1 小时时列表显示“即将过期”标签；详情页按剩余时间显示约 N 分钟、约 N 小时或具体过期时间，不使用秒级倒计时。对于密文且不归档、尚未过期的通知，客户端会通过 Notification Kit 逐条发布包含解密后真实标题和正文的本地通知；本地通知发布成功后才删除远程密文，发布失败则保留远程记录供下次同步重试。同一消息使用稳定标识更新，不会在删除重试时重复堆叠。同步完成后，客户端中的单条、多选、分组和全部删除都只操作本地副本，删除后无法从服务器恢复；分组删除覆盖该分组尚未加载到界面的全部本地通知。
...
**消息帧格式**：与 [GET /:device\_key/message](#get-device-key-message) 返回的 message 对象同构。
